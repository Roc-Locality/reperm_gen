    ...
    V: Debug + Copy + Eq + Hash + Clone + ToString + 'static,
{
    pub fn new(group: &SymmetricGroup<V>) -> Self {
        ...
        for g in group.get_set().into_iter() {
            nodemap.insert(
                g.clone(),
                ag.add_node(Box::new(g.get_function()), g.display()),
            );
        }
        //
...
mod tests {
    ...
    #[allow(unused_imports)]
    use crate::{
        graph::cayley_graph::CayleyGraph,
        group_theory::{cycle::Cycle, symmetric::SymmetricGroup},
    };
    ...
    fn group_action() {
        ...
        debug_assert_eq!(
            s5_group.get_values(group.create(bimap!())),
            Some(&vec![1, 2, 3, 4, 5])
        );
        debug_assert_eq!(
            s5_group.get_values(group.create_vec(vec![vec![1, 2]])),
            Some(&vec![2, 1, 3, 4, 5])
        );
        debug_assert_eq!(
            s5_group.get_values(group.create_vec(vec![vec![1, 5], vec![2, 4]])),
            Some(&vec![5, 4, 3, 2, 1])
        );
    }
}
